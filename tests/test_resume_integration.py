...
def garak_config(test_output_dir):
    ...
    config = {
        ...
        "reporting": {
            "report_dir": str(test_output_dir),
...
class TestResumeIntegration:
    ...
    def test_resume_after_interrupt(self, garak_config, test_output_dir):
        ...
        assert len(hitlog_run_ids) == 1, f"Multiple run_ids after resume: {hitlog_run_ids}"
        assert hitlog_run_ids.pop() == report_run_id, "Hitlog run_id inconsistent after resume"
...
class TestScoreAccuracy:
    ...
    def test_detection_with_prefixes(self, test_output_dir):
        ...
        test_cases = [
            ...
            ("IN: X5O!P%@AP[4\\PZX54(P^)7CC)7}$EICAR-STANDARD-ANTIVIRUS-TEST-FILE!$H+H*", True),
            ("The string is: X5O!P%@AP[4\\PZX54(P^)7CC)7}$EICAR-STANDARD-ANTIVIRUS-TEST-FILE!$H+H*", True),
